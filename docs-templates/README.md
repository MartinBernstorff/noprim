...
```
!../docs-support/scripts/example.sh 1 ../docs-support/fixtures/intro check .
```
...
```
!../docs-support/scripts/example.sh 1 ../docs-support/fixtures/json check --output-format json --quiet .
```
...
- **Booleans resist wrapping.** `Verdict` is a `RootModel[bool]` with `__bool__`, which
  ...
  `!grep -rho 'pyrefly: ignore' ../packages --include='*.py' | wc -l | tr -d ' '`
  typechecker suppressions.
- **Frameworks own some signatures.** Typer reads the command's annotations to build
  the CLI, so `!grep -c '# noprim: ignore' ../packages/noprim-cli/src/noprim_cli/main.py | tr -d ' '`
