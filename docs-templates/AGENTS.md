...
block. There is no way to emit raw markdown, so **a generated table is a fenced
monospace table**, never a markdown one. A literal `!` at the start of a span is
escaped by doubling it.

Anything a command needs lives in `docs-support/`, never in `docs-templates/` —
`render` mirrors its whole input tree into the output, so a script beside a template
would be copied into the repo root. Commands therefore reach for
`../docs-support/...`, since a block runs beside its own source file.

Prefer a command over prose whenever the prose is a projection of code, and a **test**
over a command whenever it is a claim about code: a command that succeeds silently
renders a "no output" marker into the document, so guards belong in
`docs-support/test_docs.py`. That file is what keeps the deny-list prose, the types
table, the config examples and this task table honest — none of which can be generated
without losing the editorial half.

Fixtures under `docs-support/fixtures/` are deliberately full of violations and are the
one thing `:noprim` excludes. `scripts/example.sh` runs one, asserts the exit code the
example claims, and pins the elapsed time — the only part of a run that is not a claim
about behaviour.
