...
- **A boolean answer is a `Verdict`, and it stays one.** Compose with `.and_()`, `.or_()`, `.negated` and `Verdict.any()`; unwrap with `.holds` only where a real `bool` is required — an `if`, or an `Arr.filter` predicate. `bool(verdict)` and `verdict.root` are the shapes this replaced: `__bool__` survives only so a stray truthiness check answers the question instead of the always-true identity of an object.
