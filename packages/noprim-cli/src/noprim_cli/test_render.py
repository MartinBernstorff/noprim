...
@pytest.mark.parametrize(
    ("surface", "expected"),
    [
        (Surface.PARAMETER, 'a.py:2:7: parameter "user_id" is annotated "str"'),
        (Surface.RETURN, 'a.py:2:7: return type is annotated "str"'),
        (Surface.ATTRIBUTE, 'a.py:2:7: attribute "user_id" is annotated "str"'),
    ],
)
def test_names_the_surface_it_found(surface: Surface, expected: str) -> None:
    violation = Violation(
