...
def default_selection() -> Selection:
    return Selection(
        frozenset(
            Arr(RULES)
            .filter(lambda rule: rule.on_by_default.holds)
...
def _selected(selectors: Selectors) -> Selection:
    return Selection(
        frozenset(
            Arr(RULES)
            .filter(lambda rule: _matches_any(rule.code, selectors).holds)
            .map(lambda rule: rule.code)
        )
    )
...
def _validated(selectors: Selectors) -> None:
    ...
    unknown = Arr(selectors.root).filter(lambda s: _matches_nothing(s).holds).to_list()
    if len(unknown) > 0:
